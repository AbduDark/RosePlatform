    ...
    public function register(Request $request)
{
    ...
    // }
        ...
        $userData['image_url'] = $user->image ? url($user->image) : null;
    ...
    public function login(Request $request)
    {
        try {
            ...
            $sessionExpiry = now()->addHours(24); // انتهاء الجلسة بعد 24 ساعة من عدم النشاط
            ...
            Log::channel('security')->info('User logged in with single session', [
                'user_id' => $user->id,
                'email' => $user->email,
                'ip' => $request->ip(),
                'device_fingerprint' => $deviceFingerprint,
...
public function logout(Request $request)
{
    try {
        ...
        // مسح التوكنز وتنظيف بيانات الجلسة
        $user->tokens()->delete();
        $user->update([
...
public function forceLogout(Request $request)
{
    try {
        ...
        // تصفير أي بيانات مرتبطة بالجلسة
        $user->update([
            'active_session_id' => null,
            'device_fingerprint' => null,
            'session_expires_at' => null,
        ...
        Log::channel('security')->info('Force logout performed via token', [
            'user_id' => $user->id,
            'email' => $user->email,
            'ip' => $request->ip(),
            'user_agent' => $request->userAgent()
